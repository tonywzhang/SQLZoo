...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(a.albumPrice)/SUM(a.trackCount) AS avgPrice
      -- , SUM(a.albumPrice) AS sumPrice, SUM(a.trackCount) AS numTracks
    FROM
      styles
    INNER JOIN
    (SELECT
      MAX(albums.price) AS albumPrice, COUNT(tracks.*) AS trackCount, albums.asin AS asin
    FROM
      albums
    INNER JOIN
      tracks ON albums.asin = tracks.album
    ...
    GROUP BY
      albums.asin
    ) a ON styles.album = a.asin
    GROUP BY
      styles.style
    ORDER BY
      avgPrice DESC, styles.style
    LIMIT 5
  SQL
end
